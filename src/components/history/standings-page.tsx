...
export function StandingsPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-5 py-10 sm:px-8">
        ...
        <table className="w-full min-w-[34rem] text-left text-sm">
          ...
          <tbody>
            {standings.map((row, index) => (
              <tr key={row.name} className={`border-t border-white/5 ${row.isBot ? "" : "bg-sky-500/10"}`}>
                <td className="px-4 py-2 font-mono text-xs text-slate-500">{index + 1}</td>
                <td className={`px-2 py-2 ${row.isBot ? "text-slate-200" : "font-semibold text-sky-200"}`}>{row.name}</td>
                <td className="px-2 py-2 text-right font-mono text-slate-400 tabular-nums">{row.matches}</td>
                <td className="px-2 py-2 text-right font-mono font-semibold text-amber-300 tabular-nums">{row.wins}</td>
                <td className="px-2 py-2 text-right font-mono text-slate-100 tabular-nums">{row.kills}</td>
                <td className="px-2 py-2 text-right font-mono text-slate-500 tabular-nums">{row.deaths}</td>
                <td className="px-2 py-2 text-right font-mono text-slate-300 tabular-nums">{kd(row).toFixed(2)}</td>
                <td className="px-4 py-2 text-right font-mono text-slate-100 tabular-nums">{row.score.toLocaleString("id-ID")}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <Link href="/" className="mt-8 inline-block text-sm text-slate-400 hover:text-slate-200">
        ← Kembali ke menu
      </Link>
    </div>
  );
