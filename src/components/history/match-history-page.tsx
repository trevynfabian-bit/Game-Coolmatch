  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-5 py-10 sm:px-8">
      ...
      <p className="mt-2 mb-6 text-sm text-slate-400">
        {all.filter((m) => !m.isTrial).length} pertandingan dan{" "}
        {all.filter((m) => m.isTrial).length} uji coba tercatat.
      </p>
            ...
            {matches.map((match) => {
              ...
              return (
                <li key={match.id}>
                  ...
                  >
                    <span
                      className={`w-20 rounded px-2 py-1 text-center text-[11px] font-semibold ${style.className}`}
                    >
                      {style.label}
                    </span>
                    <span className="min-w-0 flex-1">
                      <span className="block text-sm font-semibold text-white">
                        ...
                        {match.isTrial && match.weaponId ? (
                          <span className="font-normal text-slate-400">
                            {" "}
                            · {findWeapon(match.weaponId).name}
                          </span>
                        ) : null}
                      </span>
                      <span
                      ...
                      >
                        ...
                        {DIFFICULTY_LABEL[match.difficulty]} · {match.botCount}{" "}
                        bot
                      </span>
                    </span>
