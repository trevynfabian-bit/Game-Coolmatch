...
export function MatchDetailPage({ matchId }: { matchId: number }) {
  ...
  if (!match) {
    return (
      <div className="mx-auto max-w-2xl px-5 py-16 text-center">
        <p className="text-slate-300">Pertandingan ini tidak ditemukan.</p>
        <Link href="/riwayat/pertandingan" className="mt-3 inline-block text-sm text-emerald-300 hover:underline">
          Kembali ke riwayat
        </Link>
      </div>
    );
  ...
  const ranked = [...match.participants].sort(
    (a, b) => b.roundWins - a.roundWins || b.kills - a.kills || a.deaths - b.deaths,
  );
  const me = match.participants.find((p) => !p.isBot);
  const maxRoundKills = Math.max(1, ...match.rounds.map((round) => round.playerKills));

  return (
    <div className="mx-auto w-full max-w-3xl px-5 py-10 sm:px-8">
      <Link href="/riwayat/pertandingan" className="text-sm text-slate-400 hover:text-slate-200">
        ← Riwayat
      </Link>
      ...
      {me ? (
        <dl className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-5">
          {[
            ...
            <div key={item.label} className="rounded-xl border border-white/10 bg-slate-900/60 p-3">
              <dt className="text-[10px] tracking-[0.2em] text-slate-500 uppercase">{item.label}</dt>
              <dd className="mt-1 font-mono text-xl font-semibold text-white tabular-nums">{item.value}</dd>
            </div>
          ))}
        </dl>
      ) : null}
      ...
      <section className="mt-8" aria-labelledby="judul-ronde">
        <h2 id="judul-ronde" className="mb-3 text-[11px] tracking-[0.2em] text-slate-400 uppercase">
        ...
        <ol className="space-y-2">
          {match.rounds.map((round) => {
            ...
            return (
              <li key={round.roundNumber} className="flex items-center gap-4 rounded-lg border border-white/10 bg-slate-900/50 px-4 py-2.5">
                <span className="w-16 font-mono text-xs text-slate-500">Ronde {round.roundNumber}</span>
                <span className="min-w-0 flex-1">
                  <span className={`block text-sm ${mine ? "font-semibold text-emerald-300" : "text-slate-200"}`}>
                    {round.winnerName ?? "Seri"}
                  </span>
                  <span className="block text-[11px] text-slate-500">{REASON_LABEL[round.endedReason]}</span>
                </span>
                <span className="flex w-40 items-center gap-2">
                  <span className="h-1.5 flex-1 overflow-hidden rounded-full bg-white/10">
                    <span className="block h-full rounded-full bg-sky-400" style={{ width: `${(round.playerKills / maxRoundKills) * 100}%` }} />
                  </span>
                  <span className="w-12 text-right font-mono text-[11px] text-slate-300 tabular-nums">{round.playerKills} kill</span>
                </span>
              </li>
            );
          })}
      ...
      <section className="mt-8" aria-labelledby="judul-klasemen-akhir">
        <h2 id="judul-klasemen-akhir" className="mb-3 text-[11px] tracking-[0.2em] text-slate-400 uppercase">
          Klasemen akhir
        </h2>
        <div className="overflow-x-auto rounded-xl border border-white/10 bg-slate-950/70">
          <table className="w-full min-w-[28rem] text-left text-sm">
            ...
            <tbody>
              {ranked.map((p) => (
                <tr key={p.name} className={`border-t border-white/5 ${p.isBot ? "" : "bg-sky-500/10"}`}>
                  <td className={`px-4 py-2 ${p.isBot ? "text-slate-200" : "font-semibold text-sky-200"}`}>
                    {p.name}
                    {p.isWinner ? <span className="ml-2 text-[10px] text-amber-300">juara</span> : null}
                  </td>
                  <td className="px-2 py-2 text-right font-mono font-semibold text-amber-300 tabular-nums">{p.roundWins}</td>
                  <td className="px-2 py-2 text-right font-mono text-slate-100 tabular-nums">{p.kills}</td>
                  <td className="px-2 py-2 text-right font-mono text-slate-500 tabular-nums">{p.deaths}</td>
                  <td className="px-4 py-2 text-right font-mono text-slate-100 tabular-nums">{p.score.toLocaleString("id-ID")}</td>
                </tr>
              ))}
            </tbody>
          </table>
